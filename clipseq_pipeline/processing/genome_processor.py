...
Module containing the class for handling and processing genome data.
...
class GenomeProcessor:
    ...
    def __init__(self, genome_file: str):
        '''
        Initializes a GenomeProcessor object to extract sequence data.
    ...
    def get_chromosome_length(self, chromosome: str) -> int:
        '''
        Returns the length of a specified chromosome from the loaded genome.

        Parameters
        ----------
        chromosome : str
            Chromosome name or identifier to query.
            The name is normalized and mapped.

        Returns
        -------
        int
            Length of the specified chromosome in nucleotides (base pairs).
        '''
        chromosome = self._chromosome_name_map.get(normalize_chr(chromosome))
        chr_length = len(self.genome[chromosome])
        return chr_length